...
!------------------------------------------------------------------------------      
   ...
   !------------------------------------------------------------------------------
      ...
      write(*,*) 'recoveryMethod? 1: nodalAveraging, 2: SPR, 3: LSQ'
      read(*,*) recoveryMethod
      
      if (recoveryMethod==rec_SPR) then
         CALL initialize_Patches('listPatches.dat', &
                                     NX,NELX,IJK,G0XYZ)
         if (patchesInitialized) then
            write(*,*) 'patches imported:',nSPRPatches
         else
            write(*,*) 'error importing listPatches.dat -- required for SPR recovery'
            stop
         endif
      else
      ...
      allocate(nodalValues(nQuantDOF,nTotGrainNodes))
      gaussValues(:,:) = 0.D0
      nodalValues(:,:) = 0.D0
      ...
      write(603,*) nTotGrainNodes

      if (recoveryMethod==rec_SPR) then ! SPR
         write(*,*) 'p_deg ?'
         read(*,*) p_deg
         ...
         write(*,*) 'regularizeBoundaries? (T/F)'
      ...
      do while (val_ind < nstepTime)
         ...
         if(recoveryMethod==rec_AVG)then
            ! nodal averaging
            
            nodalValues = 0.d0   ! good to initialize before nodal recovery
            CALL nodalAveraging(nodalValues,gaussValues,nQuantDOF, &
                                G0XYZ,IJK,IJK_grainNodes, &
                                error)
         elseif(recoveryMethod==rec_SPR)then
            ! SPR method
            ...
            CALL recoverySPR(nodalValues,gaussValues,nQuantDOF, &
       p_deg,regularizeBoundaries,localNonlocal,nFailedPatches, &
         ...
         elseif (recoveryMethod==rec_LSQ) then  ! LSQ
               ...
               CALL recoveryLSQ(nodalValues,gaussValues,nQuantDOF, &
                                G0XYZ,IJK, &
...
! -------------- SUCCESSFULLY RECOVERED NODAL VALUES ------- !
         ...
         do grainNodeID=1,nTotGrainNodes
            write(603,*) nodalValues(:,grainNodeID)
         enddo
