...
!--------------------------------------------------
         ...
         if (iCrystal==HCP) then
            ...
            wpBackStress_t_NonLocal(1:4) = statev(327:330)! non-local/regularized plastic work dissipated on the back-stress...
         endif
...
!--------------------------------------------
      ...
      real(8) :: tpk_1d_tau_dev(6)              ! deviatoric part of the stress tensor at tau
      ...
      tpk_1d_tau_dev = tpk_1d_tau
      tpk_1d_tau_dev(1) = tpk_1d_tau_dev(1) - &
                         (tpk_1d_tau(1)+tpk_1d_tau(2)+tpk_1d_tau(3))/3.d0
      xtol = xtol_rel*MAXVAL(dabs(tpk_1d_tau_dev))
      !xtol = xtol_min
...
!        updates the hardening rate according to the converged stress and slip
         ...
         call make_damage(g_tau,g_t,delta_gamma, &
         dtime,xhard,tau,nslip,tot_gamma,iCrystal,iPhase,softHard,tensComp)
         ...
         do isys=1,nslip
            ...
            ! dd_g(isys) = dd_g_SH*dtime + dd_g_kGND*dtime/(g_tau(isys)-g0_bar(isys))
...
701   continue ! error handling
      ...
      elseif (iatg.eq.2)then
         umat_Err=2
         pnewdt1=PNEWDT_DECR_MULTIPLIER
         write(*,*)'g is negative, umat line 2129 @elem',curElemUMAT,'phase/crystal',iPhase,iCrystal
         return
...
!-----------------------------------------------------------------------
      ...
      use options, only: DEBUG_UMAT, UMAT_stabilize, damage_enabled, damage_gsat_enabled, damage_nonlocal
      ...
      real(8) :: hydrostaticTension, p, p2, p2_inv, R_crit
      ...
      if(iCrystal==HCP)then   ! HCP
         ...
         if (damage_nonlocal) then
            wpBackStress_used = wpBackStress_t_NonLocal
         else
            wpBackStress_used = wpBackStress_t
         endif
         nucleationFactor(:) = 0.d0
         
         if (damage_gsat_enabled) then
            hydrostaticTension = (tpk_1d(1)+tpk_1d(2)+tpk_1d(3))/3.d0
            if (hydrostaticTension < 0.d0) hydrostaticTension = 0.d0  ! only tension
            hydrostaticTension = 0.d0  ! disable the influence of external stress
            do iPlane = 1,4
               p = (5.d4*wpBackStress_used(iPlane) + abs(hydrostaticTension) ) * MPatoSI
               p2 = p**2.d0
               if (p2 /= 0.d0) then
                  p2_inv = 1.d0 / p2
   ...
   !              in terms of p only:
                  nucleationFactor(iPlane) = dexp( -(16.d0/3.d0*PI*surfaceEnergy**3.d0*p2_inv)/ (kB*TEMP))
...
!               xtemp = 1.d0 - 0.5d0*(g_alpha_t(isys_b)+g_alpha_tau(isys_b))/xs(isys_b)
      ...
      subroutine make_damage(g_alpha_tau,g_alpha_t,delta_gamma,  &
        dtime,xhard,tau,nslip,tot_gamma,iCrystal,iPhase,softHard,tensComp)
        
      use material_Ti6242
      use options, only: damage_enabled, damage_nonlocal
      ...
      if(damage_enabled.and.iCrystal==HCP)then   ! HCP
         ...
         slipSystemDamage(1) = MIN(abs(wpBackStress_used(1))/slipSystemLife,1.d0) &
                               **slipSystemExhaustionExp

         !slipSystemDamage(1) = MIN(slipSystemDamage(1),1.d0)   ! can do up to 1
         ...
         do isys=4,6
            ...
            !-- dislocation interaction energy based
            slipSystemDamage(isys-2) = MIN(wpBackStress_used(isys-2)/slipSystemLife,1.d0)&
                                       **slipSystemExhaustionExp
...
!----------------------------------------------------------------
      ...
      real(8), parameter :: kB=1.3806488d-23  ! boltzmann constant (J/K)
      real(8), parameter :: TAU_LIMIT = 1.5d0
...
!-------------------------------------------------------------
      ...
      do isys=1,nslip
         ...
         if(USE_POWER_LAW) then
            ...
            if (atg < TAU_LIMIT) then
               ...
               delta_gamma(isys)=a_dot(isys)*dtime*  &
                  (TAU_LIMIT**(1.d0/rate_exp(isys))+ &
      (1.d0/rate_exp(isys))*TAU_LIMIT**(1.d0/rate_exp(isys)-1.d0)* &
                                                 (atg-TAU_LIMIT))*  &
                                 dsign(1.d0,tau(isys))            
            endif
         else
      ...
      dGamma_dTau(isys)=a_dot(isys)*dtime*rate_inv*TAU_LIMIT**(rate_inv-1.d0)  &
                        *g_inv
               endif
            else
